<template>
  <div class="container">
    <div>
      ...
      <div>
        <h2 class="subtitle">
          Deposit Any Token and Withdraw with Master Token
       </h2>
       <div class="content">
          <a
            target="_blank" 
          ...
          <br>
          <br>
          <label for="MasterTokenAddress">
            Cut And Paste the ERC20 Address (Master Token) To be Used for Withdrawal:
          </label>
          <input 
            v-model="MasterTokenAddress"
            @change="UpdateMasterTokenAddress()"
          />
          <div
            v-if="ethereumAddress && !contractFactoryAddress" 
            class="links">
            <a
              @click="launchDepositContract()"
              class="button--green"
            >
              Launch Multi-Deposit Contract
            </a>
          </div>
          <br>
...
export default {
  ...
  async mounted() {
    ...
    if (localStorage.DepositlaunchHash) this.launchHash = localStorage.DepositlaunchHash
    if (localStorage.MasterTokenAddress) this.MasterTokenAddress = localStorage.MasterTokenAddress
    if (localStorage.DepositcontractFactoryAddress) this.contractFactoryAddress = localStorage.DepositcontractFactoryAddress
  },
  methods: {
    ...
    UpdateMasterTokenAddress(){
      localStorage.MasterTokenAddress = this.MasterTokenAddress
    },
    async launchDepositContract(){

      let factory = new this.ethers.ContractFactory(this.abi, this.bytecode, this.signer);

      
      let contract = await factory.deploy(this.MasterTokenAddress);

      this.showLoading = true
      await contract.deployed()
      this.showLoading = false

      this.contractFactoryAddress = localStorage.DepositcontractFactoryAddress = contract.address
    },
    async launchToken(){
      ...
      this.recipientAddress = localStorage.DepositrecipientAddress = recipient.address
