...
module.exports = function(deployer) {
  ...
  deployer
    .then(async () => {
      ...
      console.log(
        "CryptoravesToken Contract Address: " + CryptoravesTokenAddress
      );
      console.log(
        "\n*************************************************************************\n"
      );
      await fs.appendFile(
        outputPath,
        '"CryptoravesToken":"' + CryptoravesTokenAddress + '",',
        function(err) {
          if (err) throw err;
        }
      );
      ...
      await fs.appendFile(
        outputPath,
        '"TokenManagement":"' + instance.address + '",',
        function(err) {
          if (err) throw err;
        }
      );
    ...
    .then(async () => {
      ...
      try {
        await fs.writeFileSync(
          "build/contracts/CryptoravesToken.json",
          JSON.stringify(CryptoravesTokenJson)
        );
      } catch (err) {
        console.error(err);
      }
