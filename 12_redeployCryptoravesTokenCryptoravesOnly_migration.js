...
const ValidatorInterfaceContract = artifacts.require(
  "ValidatorInterfaceContract"
);
...
try {
  dataFile =
    homedir +
    "/token-game/lambda-functions/TwitterEndpointV1/test-data/realUserDataCryptoravesTokensOnly.json";
} catch (e) {
  console.log(e);
  console.log(
    "realUserData.json file not found. Be sure token-game is cloned in your HOME dir"
  );
}
...
async function sendETHtoAccountZero(accounts) {
  await web3.eth.sendTransaction({
    to: accounts[0],
    from: accounts[accountsCounter + 1],
...
module.exports = function(deployer, network, accounts) {
  let rawdata = fs.readFileSync(
    "/tmp/" + deployer.network + "-contractAddresses.json"
  );
  let addresses = JSON.parse(rawdata);
  console.log(addresses);
  deployer.then(async () => {
    validatorInstance = await ValidatorInterfaceContract.at(
      addresses["ValidatorInterfaceContract"]
    );
    ...
    const outputPath =
      "/tmp/" + deployer.network + "-tokenDredeployErrorsistro.txt";
    try {
    ...
    for await (const line of rl) {
      ...
      let isExport = false;
      if (
        record["twitterIdFrom"] == "LAUNCH" ||
        record["twitterIdFrom"] == "IMPORT"
      ) {
      ...
      if (
        record["L1Address"] &&
        !mappedAccounts.includes(record["L1Address"])
      ) {
        //map account
        try {
          res = await validatorInstance.validateCommand(
            [
              record["twitterIdFrom"],
              record["twitterIdTo"],
              record["twitterIdThirdParty"],
              0,
              record["decimalPlaceLocation"],
              record["tweetId"]
            ],
            [
              record["twitterHandleFrom"],
              record["twitterHandleTo"],
              record["ticker"],
              record["_platformName"],
              "mapaccount",
              record["_fromImgUrl"],
              record["L1Address"]
            ]
          ...
          await fs.appendFile(
            outputPath,
            e.toString() + JSON.stringify(record) + "\r\n",
            function(err) {
              console.log(record);
              console.log(e);
            }
